  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <div className="space-y-2">
        <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">{baslik}</h1>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="space-y-0.5">
          <p className="text-xs font-black text-amber-950">Nihai metin hazırlanıyor</p>
      ...
      {bolumler.map((bolum) => (
        <section key={bolum.baslik} className="space-y-3">
          <h2 className="text-sm font-black text-slate-900">{bolum.baslik}</h2>
          <div className="space-y-2.5">
            {bolum.satirlar.map((satir, i) =>
              typeof satir === 'string' ? (
                ...
                >
                  <span className="text-xs font-black text-slate-900 sm:w-44 shrink-0">
                    {satir.ad}
                  </span>
      ...
      <div className="border-t border-slate-100 pt-6 flex flex-wrap gap-3">
        {[
          ...
          <Link
            key={yol}
            href={yol}
            className="text-[11px] font-bold text-indigo-600 hover:text-indigo-700 hover:underline rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600"
          >
            {ad}
          </Link>
        ))}
