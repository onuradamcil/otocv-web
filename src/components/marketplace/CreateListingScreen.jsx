// =========================================================================
// OTO-CV İLAN VERME SAYFASI: ENTERPRISE FULL-PAGE İLAN SİHİRBAZI (CreateListingScreen.jsx)
// İşlev: Referans tasarıma birebir uygun sadeleştirilmiş sütun kütüğü,
//        sabit sütun başlıkları (1. Kategori, 2. Model Yılı vs.),
//        ilk açılışta Kategori Seçim zorunluluğu, dinamik breadcrumb akışı
//        ve ok ucu (chevron tab) poligon stil entegrasyonu.
// =========================================================================
...
export default function CreateListingScreen({ onBack, onSuccess, user }) {
  // =========================================================================
  // 1. BLOK: REAKTİF STATE HAFIZASI VE KATALOG SEÇİM DÖNGÜLERİ
  // =========================================================================
  ...
  // Fotoğraf Yükleme ve Sürükle-Bırak State'leri
  const [photos, setPhotos] = useState([]);
  const [isUploading, setIsUploading] = useState(false);
  const [isDragging, setIsDragging] = useState(false);

  // Sütunlu Araç Seçim Kütüğü Hafızası (Sıfırdan Başlar)
  ...
  // Kullanıcının Profilindeki Paket Tipini Veritabanından Çeken Sensör
  ...
  // =========================================================================
  return (
    <div className="min-h-screen bg-[#F2F4F7] pb-24 text-slate-900 select-none">
          ...
          <div className="flex items-center gap-2.5 shrink-0 self-end sm:self-auto">
            <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200/80 px-3 py-1.5 rounded-lg">
              ...
              <span className="text-xs font-black text-indigo-600 bg-white px-2 py-0.5 rounded border border-slate-200/60 shadow-2xs font-mono">
                {userPackage.remainingQuota} Adet
              </span>
            </div>
...
{/* =========================================================================
    ...
    İşlev: Büyütülmüş ana başlık, her sütunun tepesinde soft numaralı Inter rozetler,
           milimetrik eşitlenmiş sütun genişlikleri (w-44) ve ok ucu seçim tabları.
   ========================================================================= */}
        ...
        <div className="w-44 border-r border-slate-100 pr-1 shrink-0 space-y-1 overflow-y-auto max-h-[310px] animate-fadeIn">
          ...
          {packagesList.map(p => {
            ...
            return (
              <div 
                key={p.id}
                onClick={() => { setSelectedPackage(p); setIsFinalConfirmed(false); }}
                style={isSelected ? arrowTabStyle : {}}
                className={`px-3 py-2 text-xs font-semibold cursor-pointer transition-all flex items-center ${
                  isSelected 
                    ? 'bg-[#EEF1F6] text-slate-900 font-bold pr-4' 
          ...
          <div className="space-y-2">
            ...
            >
              <div className="flex items-start gap-2">
                <input type="radio" checked={isFinalConfirmed} onChange={() => setIsFinalConfirmed(true)} className="mt-0.5" />
                <div>
